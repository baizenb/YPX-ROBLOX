...
-- Version: v1.15 (Auto-Incremented +0.01)
-- Developer: ypx
-- Architecture: Luxury Multi-Adapter Engine with Safe Splash & Silk-Smooth Drag
-- ===================================================
...
-- API 适配器 ( Multi-Executor Environment Adapters )
...
-- [1. Hub Global Configuration]
local Config = {
    HubVersion = "1.15",                               -- Hub Version ( v1.15 )
    KeySystemEnabled = true,                           -- Enable Key Verification ( 开启卡密验证 )
    UserKey = "ypx-vip-2026",                          -- Correct Key ( 正确卡密 )
    ValidDurationHours = 24,                           -- Validity Duration in Hours ( 有效期24小时 )
    KeyLink = "https://lootdest.org/s?kjLRWzSa",        -- LootLabs Key Link ( LootLabs链接 )
    DiscordLink = "https://discord.gg/szQ573FFQ",      -- Discord Community Link ( 社区链接 )
    SaveFileName = "YPX_Hub_KeyData.json"              -- Local Save File ( 本地保存文件名 )
}

-- [2. Anti-Deduplication ( 防重复多开，确保只存活一个UI框架 )]
...
-- [3. Silk-Smooth Drag Engine ( 丝滑流畅不卡顿的拖拽引擎，完美修复所有UI拖动 )]
...
-- ===================================================
-- [4. Local Storage Key Gate & Persistence ( 本地缓存验证 )]
-- ===================================================
...
-- ===================================================
-- [5. Splash Loading UI ( 修复版开屏引导与安全倒计时动画 )]
-- ===================================================
local function ShowSplashLoadingScreen()
    ...
    StatusLabel.Text = "正在初始化多端引擎与安全验证 ( Initializing ... )"
    ...
    CountdownLabel.Text = "3 秒后启动卡密验证界面 ( Launching in 3s )..."
    ...
    task.spawn(function()
        local success, err = pcall(function()
            for i = 3, 1, -1 do
                CountdownLabel.Text = i .. " 秒后启动卡密验证界面 ( Launching in " .. i .. "s )..."
                task.wait(1)
            end
        end)
        
        -- 确保无论如何都能销毁开屏UI并安全流转下一步
        pcall(function()
            SplashFrame:Destroy()
        end)
...
-- ===================================================
-- [6. Luxury Login / Key Verification UI ( 卡密验证界面 )]
-- ===================================================
ShowKeyVerificationWindow = function()
    ...
    GetKeyBtn.Text = "🔑 获取卡密 ( Get Key Link )"
    ...
    GetKeyBtn.MouseButton1Click:Connect(function()
        ...
        GetKeyBtn.Text = "✓ 链接已复制 ( Link Copied )"
        task.wait(2)
        GetKeyBtn.Text = "🔑 获取卡密 ( Get Key Link )"
    end)
    ...
    KeyInput.Text = ""
    KeyInput.PlaceholderText = "在此粘贴卡密 ( Paste Key Here )..."
    ...
    StatusText.Text = "提示 ( Notice )：验证成功后有效期 24 小时 ( Valid 24h )"
    ...
    DiscordBtn.Text = "💬 加入 Discord 社区 ( Join Discord )"
    ...
    VerifyBtn.MouseButton1Click:Connect(function()
        if KeyInput.Text == Config.UserKey then
            StatusText.Text = "✓ 验证成功！正在进入 ( Success! )"
            ...
            StatusText.Text = "❌ 卡密错误 ( Invalid Key )"
...
-- ===================================================
-- [7. Luxury Main Hub UI & Floating Ball ( 豪华悬浮球与主框架 )]
-- ===================================================
...
-- ===================================================
-- [8. Component Factory & Non-Blocking UI Framework ( UI组件工厂 )]
-- ===================================================
local TitleText
...
-- ===================================================
-- [8. Universal Hub Interface ( 通用属性模块 )]
-- ===================================================
...
-- ===================================================
-- [9. Anime Incremental Exclusive Interface ( 专属于该游戏的接口 )]
-- ===================================================
local function LoadAnimeIncrementalInterface()
    TitleText.Text = "ANIME INCREMENTAL ( 动漫增量 )"

    local RemotesPath = ReplicatedStorage.Packages._Index["leifstout_networker@0.3.1"].networker._remotes
    ...
    -- 【全属性极速 Max 连点开关】调快频率至 0.02 秒并对每个属性满级连点
...
-- ===================================================
-- [11. Engine Entrypoint & Dual-Gate Architecture]
-- ===================================================
